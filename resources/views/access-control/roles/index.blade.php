    ...
    <div class="content">
        <div class="card mt-5 m-2 shadow-sm">
            <div class="card-header card-header-common p-3 pb-2">
                <span class="me-2 text-info fas fa-user-tie" style="stroke-width:2;"></span><b>Roles</b>
                <span class="float-end">
                    <a href="{{ route('roles.create') }}" class="btn btn-sm btn-phoenix-secondary rounded-pill me-0" type="button">Add New
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-sm fs-9 mb-0">
                            ...
                            @foreach($roles as $role)
                                <tr>
                                    <td class="align-middle ps-3 id"><span class="badge badge-phoenix badge-phoenix-primary">{{ $role->id }}</span></td>
                                    <td class="align-middle name"><strong>{{ $role->name }}</strong></td>
                                    <td class="align-middle guard_name">{{ $role->guard_name }}</td>
                                    <td class="align-middle created_at">{{ $role->created_at }}</td>
                                    <td class="align-middle updated_at">{{ $role->updated_at }}</td>
